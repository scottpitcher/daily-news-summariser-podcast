...
export function BriefingDetail({ briefing, onBack }: BriefingDetailProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-[#e7e0c9] shadow-sm p-6">
      <button
        onClick={onBack}
        className="text-sm font-medium text-gray-500 hover:text-[#283618] transition-colors mb-4"
      >
        ← Back to briefings
      </button>
      ...
      <h2 className="text-[#283618] text-xl font-bold pb-3 mb-5 border-b border-[#e7e0c9]">
      ...
      <div className="space-y-7">
        {briefing.articles.map((article) => (
          <div key={`${article.rank}-${article.title}`}>
            <h3 className="text-[#283618] text-base font-bold mb-1.5">{article.headline}</h3>
            ...
            {article.soWhat && (
              <p className="text-sm text-gray-700 leading-relaxed mb-2.5 bg-[#bc6c25]/[0.08] rounded-lg px-3 py-2">
                <span className="font-bold text-[#bc6c25]">So what? </span>
                {article.soWhat}
              </p>
            )}
            <p className="text-xs text-gray-400">
              ...
              {article.sourceUrl ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="underline hover:text-[#283618] transition-colors"
                >
