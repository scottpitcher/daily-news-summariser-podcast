...
function App() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 relative">
      <div className="hidden xl:block absolute top-8 bottom-8 right-full mr-6 w-64 rounded-xl border border-[#e0dcd0] overflow-hidden">
        <DateSidebar dates={availableDates} selectedDate={date} onSelectDate={selectDate} />
      </div>

      <button
        onClick={() => setSidebarOpen(true)}
        className="xl:hidden mb-4 inline-flex items-center gap-2 bg-white border border-[#e0dcd0] rounded-lg px-3.5 py-2 text-sm font-medium text-[#211f1c] hover:bg-[#ece8dc] transition-colors"
      >
