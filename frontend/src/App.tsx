...
function App() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 relative">
      <div className="hidden xl:block absolute top-8 bottom-8 right-full mr-6 w-64 rounded-2xl border border-[#c9ccc6] shadow-sm overflow-hidden">
        <DateSidebar dates={availableDates} selectedDate={date} onSelectDate={selectDate} />
      </div>

      <button
        onClick={() => setSidebarOpen(true)}
        className="xl:hidden mb-4 inline-flex items-center gap-2 bg-white border border-[#c9ccc6] rounded-lg px-3.5 py-2 text-sm font-medium text-[#16425b] shadow-sm hover:bg-[#d9dcd6] transition-colors"
      >
